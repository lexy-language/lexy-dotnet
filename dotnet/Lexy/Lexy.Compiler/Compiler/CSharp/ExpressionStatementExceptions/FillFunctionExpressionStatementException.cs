...
namespace Lexy.Compiler.Compiler.CSharp.ExpressionStatementExceptions
{
    internal class FillFunctionExpressionStatementException : IExpressionStatementException
    {
        ...
        {
            if (!(expression is VariableDeclarationExpression assignmentExpression)) throw new InvalidOperationException("expression should be AssignmentExpression");
            ...
            if (!(functionCallExpression.ExpressionFunction is FillParametersFunction fillParametersFunction)) throw new InvalidOperationException("functionCallExpression.ExpressionFunction should be FillParametersFunction");

            return FillStatementSyntax(assignmentExpression.Name, fillParametersFunction.Type, fillParametersFunction.Mapping);
        }
            ...
            IEnumerable<Mapping> mappings)
        {
            ...
            var initialize = SyntaxFactory.ObjectCreationExpression(Types.Syntax(type))
